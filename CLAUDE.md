...
Monorepo for an AI skill creation and management platform. Three apps (api, web, mcp-server) share five packages (shared, db, auth, claude-client, skill-engine). Skills are defined in SKILL.md format (YAML frontmatter + markdown sections).
...
@accrue-ai/shared          (leaf: types, constants, utils — no internal deps)
    ...
    └── @accrue-ai/skill-engine (parse, create, validate, iterate, compare, package, import)
            ...
            ├── apps/mcp-server  (MCP stdio — depends on db, shared, skill-engine, claude-client)
...
docker/               Dockerfiles + docker-compose.yml (postgres, api, web, mcp-server)
